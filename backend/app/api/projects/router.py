...
from app.api.maps.models import MapGroupAccess, MapModel
...
@router.get(
    "", response_model=list[ProjectRead], summary="List all projects accessible to current user"
)
...
@router.get(
    "/{project_id}",
    response_model=ProjectRead,
    summary="Get single project workspace by project_id",
)
...
@router.delete(
    "/{project_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete project workspace"
)
...
@router.post(
    ...
    status_code=status.HTTP_201_CREATED,
    summary="Publish a new map from this project workspace",
)
