...
export const ProfileCard: React.FC<ProfileCardProps> = ({
  ...
  return (
    <button
      ...
      type="button"
      className={`group relative flex flex-col items-center rounded-2xl p-4 text-center transition-all duration-200 ${
        isActive
    ...
    >
      ...
      <div
        className={`mt-1 line-clamp-2 text-xs ${isActive ? "text-white/80" : "text-[var(--text-tertiary)]"} `}
      >
        {t(profile.descKey)}
      </div>
      {isActive ? (
