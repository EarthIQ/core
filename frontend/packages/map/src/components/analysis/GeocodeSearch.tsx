...
export const GeocodeSearch: React.FC<GeocodeSearchProps> = ({
  ...
  const nominatimGeocoder = useCallback(
    async (searchQuery: string): Promise<GeocodeResult[]> => {
      ...
      return data.map((item: any) => ({
        ...
        coordinates: [parseFloat(item.lon), parseFloat(item.lat)] as [
          number,
          number,
        ],
            ...
            {results.map((result, index) => (
              <div
                ...
                style={{
                  ...
                  cursor: "pointer",
                  backgroundColor:
                    selectedIndex === index ? "#f0f7ff" : "transparent",
