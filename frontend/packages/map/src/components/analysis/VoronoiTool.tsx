...
export const VoronoiTool: React.FC<VoronoiToolProps> = ({
  ...
  const getGeoJSON = useCallback(
    (src: string | FeatureCollection): FeatureCollection | null => {
  ...
  const executeVoronoi = useCallback(() => {
    ...
    try {
      ...
      if (!voronoiBbox) {
        voronoiBbox = turf.bbox(pointCollection) as [
          number,
          number,
          number,
          number,
      ...
      voronoiPolygons.features = voronoiPolygons.features
        .map((feature, index) => {
      ...
      const error =
        err instanceof Error ? err : new Error("Voronoi generation failed");
      setError(error);
      onError?.(error);
    } finally {
