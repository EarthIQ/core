...
export const FeatureTable: React.FC<FeatureTableProps> = ({
  ...
  const handleRowSelect = useCallback(
    (feature: GeoJSON.Feature, checked: boolean) => {
      ...
      if (highlightOnSelect && map && isLoaded) {
        ...
        const selectedFeatures = data.features.filter(
          (f) =>
            selectedIds.has(f.id || f.properties?.id) ||
            (checked && (f.id || f.properties?.id) === id)
        );
      ...
      onSelect?.(
        data.features.filter((f) => selectedIds.has(f.id || f.properties?.id))
      );
    },
                ...
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      setSelectedIds(
                        new Set(
                          processedData.map((f) => f.id || f.properties?.id)
                        )
                      );
                    } else {
            ...
            {columns.map((col) => (
              <th
                ...
                style={{
                  ...
                  textAlign: "left",
                  cursor:
                    col.sortable !== false && sortable ? "pointer" : "default",
                  width: col.width,
                  userSelect: "none",
                }}
                onClick={() =>
                ...
                {selectable ? (
                  ...
                  >
                    <input
                      ...
                      type="checkbox"
                      onChange={(e) =>
                        handleRowSelect(feature, e.target.checked)
                      }
                    />
                  </td>
                ) : null}
