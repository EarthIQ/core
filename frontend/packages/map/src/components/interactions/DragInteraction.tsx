...
export const DragInteraction: React.FC<DragInteractionProps> = ({
  ...
  const [draggedFeature, setDraggedFeature] = useState<GeoJSON.Feature | null>(
    null
  );
  ...
  const currentOffsetRef = useRef<{ lng: number; lat: number }>({
    lng: 0,
    lat: 0,
  });
  ...
  const updatePreview = useCallback(
    (feature: GeoJSON.Feature, deltaLng: number, deltaLat: number) => {
  ...
  const handleMouseMove = useCallback(
    (e: any) => {
      ...
      if (draggedFeature && startPointRef.current) {
        ...
        if (draggableFeatures && draggableFeatures.length > 0) {
          const featureId =
            draggableFeatures[0].id ?? draggableFeatures[0].properties?.id;
