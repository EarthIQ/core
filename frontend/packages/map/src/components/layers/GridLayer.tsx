...
export const GridLayer: React.FC<GridLayerProps> = ({
  ...
  const [gridCells, setGridCells] = useState<GeoJSON.FeatureCollection | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const generateGrid = () => {
      ...
      const cellsWithData = grid.features
        .map((cell) => {
          ...
          switch (colorAggregation) {
            case "SUM":
              colorValue = pointsInCell.features.reduce(
                (sum, p) => sum + (p.properties?.colorWeight || 0),
                0
              );
              break;
            case "MEAN":
              colorValue =
                pointsInCell.features.reduce(
                  (sum, p) => sum + (p.properties?.colorWeight || 0),
                  0
                ) / count;
              break;
            case "MAX":
              colorValue = Math.max(
                ...pointsInCell.features.map(
                  (p) => p.properties?.colorWeight || 0
                )
              );
              break;
            case "MIN":
          ...
          switch (elevationAggregation) {
            case "SUM":
              elevationValue = pointsInCell.features.reduce(
                (sum, p) => sum + (p.properties?.elevationWeight || 0),
                0
              );
              break;
            case "MEAN":
              ...
              break;
            case "MAX":
              elevationValue = Math.max(
                ...pointsInCell.features.map(
                  (p) => p.properties?.elevationWeight || 0
                )
              );
              break;
            case "MIN":
              elevationValue = Math.min(
                ...pointsInCell.features.map(
                  (p) => p.properties?.elevationWeight || 0
                )
              );
  ...
  useEffect(() => {
    ...
    const colorValues = gridCells.features.map(
      (f) => f.properties?.colorValue || 0
    );
    ...
    colorRange.forEach((color, i) => {
      const value =
        minValue + (maxValue - minValue) * (i / (colorRange.length - 1));
