...
export const WFSSource: React.FC<WFSSourceProps> = ({
  ...
  const [_capabilities, setCapabilities] = useState<WFSCapabilities | null>(
    null
  );
  ...
  const buildGetFeatureUrl = useCallback(
  ...
  const fetchWFSCapabilities =
    useCallback(async (): Promise<WFSCapabilities | null> => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch WFS capabilities: ${response.status}`
          );
        }
        ...
        const capabilities: WFSCapabilities = {
          version,
          title:
            xml.querySelector("ServiceIdentification > Title, Service > Title")
              ?.textContent || undefined,
          abstract:
            xml.querySelector(
              "ServiceIdentification > Abstract, Service > Abstract"
            )?.textContent || undefined,
          featureTypes: [],
          outputFormats: [],
        };
        ...
        featureTypeNodes.forEach((node) => {
          const featureType: WFSFeatureType = {
            ...
            defaultCRS:
              node.querySelector("DefaultCRS, DefaultSRS")?.textContent ||
              undefined,
          };
          ...
          if (otherCRSNodes.length > 0) {
            featureType.otherCRS = Array.from(otherCRSNodes)
              .map((n) => n.textContent || "")
              .filter(Boolean);
          }
          ...
          const bboxNode = node.querySelector(
            "WGS84BoundingBox, LatLongBoundingBox"
          );
          if (bboxNode) {
            const lowerCorner = bboxNode
              .querySelector("LowerCorner")
              ?.textContent?.split(" ");
            const upperCorner = bboxNode
              .querySelector("UpperCorner")
              ?.textContent?.split(" ");
          ...
          if (keywordNodes.length > 0) {
            featureType.keywords = Array.from(keywordNodes)
              .map((n) => n.textContent || "")
              .filter(Boolean);
          }
        ...
        const formatNodes = xml.querySelectorAll(
          'OutputFormat, Parameter[name="outputFormat"] Value'
        );
        capabilities.outputFormats = Array.from(
          new Set(
            Array.from(formatNodes)
              .map((n) => n.textContent || "")
              .filter(Boolean)
          )
        );
  ...
  const fetchData = useCallback(
    async (currentBbox?: [number, number, number, number]) => {
      ...
      try {
        ...
        if (outputFormat.includes("json")) {
          ...
          throw new Error(
            "GML parsing not implemented. Use application/json outputFormat."
          );
        }
        ...
        if (generateId) {
          featureCollection.features = featureCollection.features.map(
            (feature, index) => ({
              ...feature,
              id:
                feature.id ?? feature.properties?.id ?? `${typeName}-${index}`,
            })
          );
        }
...
export const useWFSTransaction = (options: WFSTransactionOptions) => {
  ...
  const buildTransactionXML = useCallback(
    (
      ...
      if (operation === "Insert") {
        ...
        const propertyXML = Object.entries(updateProperties)
          .map(
            ([name, value]) =>
  ...
  const executeTransaction = useCallback(
    async (
      ...
      try {
        ...
        // Check for exception
        const exception = responseXML.querySelector(
          "ExceptionReport, ServiceException"
        );
        if (exception) {
          return {
            success: false,
        ...
        const summary = responseXML.querySelector(
          "TransactionSummary, TransactionResult"
        );

        return {
          success: true,
          totalInserted: parseInt(
            summary?.querySelector("totalInserted, InsertResults")
              ?.textContent || "0"
          ),
          totalUpdated: parseInt(
            summary?.querySelector("totalUpdated")?.textContent || "0"
          ),
          totalDeleted: parseInt(
            summary?.querySelector("totalDeleted")?.textContent || "0"
          ),
          insertedFeatureIds: Array.from(
            responseXML.querySelectorAll(
              "InsertResult FeatureId, InsertResults Feature"
            )
          )
            .map(
              (node) =>
                node.getAttribute("fid") || node.getAttribute("gml:id") || ""
            )
            .filter(Boolean),
        };
      } catch (error) {
  ...
  const insertFeatures = useCallback(
  ...
  const updateFeatures = useCallback(
    async (properties: Record<string, any>, filterXML: string) => {
  ...
  switch (geometry.type) {
    ...
    case "Polygon": {
      const exterior = geometry.coordinates[0]
        .map((c) => c.join(","))
        .join(" ");
      const interiors = geometry.coordinates
        .slice(1)
        .map(
          (ring) =>
