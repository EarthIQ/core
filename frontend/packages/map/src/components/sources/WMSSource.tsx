...
export const WMSSource: React.FC<WMSSourceProps> = ({
  ...
  const [capabilities, setCapabilities] = useState<WMSCapabilities | null>(
    null
  );
  ...
  const fetchWMSCapabilities =
    useCallback(async (): Promise<WMSCapabilities | null> => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch WMS capabilities: ${response.status}`
          );
        }
        ...
        // Parse capabilities
        const capabilitiesNode = xml.querySelector(
        ...
        const parsedCapabilities: WMSCapabilities = {
          ...
          abstract:
            xml.querySelector("Service > Abstract")?.textContent || undefined,
          layers: [],
          formats: [],
          crs: [],
        };
        ...
        layerNodes.forEach((layerNode) => {
          const layerInfo: WMSLayerInfo = {
            ...
            abstract:
              layerNode.querySelector("Abstract")?.textContent || undefined,
        ...
        parsedCapabilities.formats = Array.from(formatNodes)
          .map((node) => node.textContent || "")
          .filter(Boolean);

        // Parse CRS
        const crsNodes = xml.querySelectorAll("Layer > CRS, Layer > SRS");
        parsedCapabilities.crs = Array.from(
          new Set(
            Array.from(crsNodes)
              .map((node) => node.textContent || "")
              .filter(Boolean)
