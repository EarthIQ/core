...
> {
  ...
  /** Line height */
  lineHeight?:
    "none" | "tight" | "snug" | "normal" | "relaxed" | "loose" | number;
  /** Letter spacing */
  ...
  (
    ...
    const computedStyles = useMemo((): React.CSSProperties => {
      ...
      if (letterSpacing) {
        styles.letterSpacing =
          letterSpacingValues[letterSpacing] || letterSpacing;
      }
    ...
    return React.createElement(
      Element,
      {
        ref,
        className:
          `${className || ""} ${responsiveClassName}`.trim() || undefined,
...
export const Label: React.FC<
  Omit<TextProps, "variant"> & { required?: boolean; htmlFor?: string }
...
> = ({
  ...
  const linkStyles: React.CSSProperties = {
    color,
    textDecoration:
      underline === "always" || (underline === "hover" && isHovered)
        ? "underline"
        : "none",
    cursor: "pointer",
